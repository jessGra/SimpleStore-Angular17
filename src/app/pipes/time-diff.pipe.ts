  ...
  transform(value: any, args?: any): any {
    if (value) {
      ...
      for (let i of allInterval) {
        ...
        let toReturn: string = '';
        // calculation shown for for 2hour:51 minute = 171 minute = 10260 second
        if (counter > 0) {
          toReturn += this.calculateTime(counter, i); // this will give 2 hours
          let timeLeft =
            seconds - counter * intervals[i as keyof typeof intervals]; // 3060 second
          ...
          counter = Math.floor(
            timeLeft / intervals[i as keyof typeof intervals]
          ); // 3060 second = 51
          toReturn =
